...
bird_baths_count %>% 
  ...
  ggplot(aes(x=as.factor(id), y=n, fill = bird_type)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  theme(
    ...
    plot.margin = unit(rep(-1.5,4), "cm"),      # Adjust the margin to make in sort labels are not truncated!
  ...
  # Add the labels, using the label_data dataframee that we have created before
  geom_text(data=label,
            ...
            angle= label$angle, 
            inherit.aes = FALSE ) +
  # Add main text within the circle
  geom_text(
  ...
  #Add sub text within the circle
  geom_text(
    aes(x = 0, y = -200, label = str_wrap("Occurrence of Australian bird species at bird baths in private gardens", 25)),
    size = 3,
    family = "Segoe UI",
    color = "tomato3"
  ) +
